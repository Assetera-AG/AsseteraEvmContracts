...
| Token standard | Escrow is **measured at the pull** (`core/EscrowPull.sol`, since 4.2.0): the contract reads its own balance before and after `transferFrom` and records the delta. An order opens with `remainingQuantity = received - escrowedFee` and emits `OrderEscrowShort` when that is less than `sellAmount` (the escrowed fee stays whole, `sellAmount` stays the price basis); an offer's proposer is credited the same way on their asset leg since 4.4.0 (`OfferEscrowShort`; the counterparty sees the booked leg before accepting), while a party delivering at the moment of a trade — the accepting leg, the asset on a buy-side fill (routed through the exchange since 4.3.0), a proposer's currency leg with its fee — must arrive whole or the call reverts `EscrowPullShort(requested, received)`. So a **fee-on-transfer** token cannot overstate the pool. Payouts still move the recorded figure and the token taxes them again, which the recipient bears. **Rebasing** tokens remain unsupported: a balance that moves after placement is invisible to a pull measurement, so a negative rebase still desyncs the pool. The backend must not list a rebasing token; listing one would require a contract upgrade |
